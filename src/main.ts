...
function getGitExecOptions(options?: exec.ExecOptions): exec.ExecOptions {
  ...
  if (cancelStallingEnabled) {
    if (!process.env.GIT_HTTP_LOW_SPEED_LIMIT) {
      gitEnv.GIT_HTTP_LOW_SPEED_LIMIT = process.env.NSC_CHECKOUT_ACTION_GIT_HTTP_LOW_SPEED_LIMIT || '1000'
    }
    if (!process.env.GIT_HTTP_LOW_SPEED_TIME) {
      gitEnv.GIT_HTTP_LOW_SPEED_TIME = process.env.NSC_CHECKOUT_ACTION_GIT_HTTP_LOW_SPEED_TIME || '180'
    }
  }
